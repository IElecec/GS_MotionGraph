...
- `--image-manifest`: optional image manifest from `render_image_library.py`; enables image playback in that HTML view
...
```bash
python visualize_motion_graph.py -g outputs/jyf/motion_graph.json -o outputs/jyf/motion_graph.html
```
...
- `--image-manifest`: optional manifest generated by `render_image_library.py`; enables frame-image playback inside the HTML
...
After the image library is ready, pass the manifest into the HTML visualizer:

```bash
python visualize_motion_graph.py \
  -g "outputs\jyf\motion_graph.json" \
  -o "outputs\jyf\motion_graph.html" \
